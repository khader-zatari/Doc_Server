...
public class DocControllerTests {
    ...
    void getDocument_GetContentExistsDoc_Works() throws InterruptedException {
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile8", INodeType.FILE), token, userId);
    ...
    void sendUpdatedText_ProvideRightParam_Works() throws InterruptedException {
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile6", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
        String content = "k";
        docController.getDocument(doc.getId(), token, userId);
        docController.sendUpdatedText(doc.getId(), new ManipulatedTextDTO(userId, UpdateTypeDTO.APPEND, content, 0, 0));
    ...
    void sendUpdatedText_ProvideWrongParam_Exception() throws InterruptedException {
        ...
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile9", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
        docController.getDocument(doc.getId(), token, userId);
    ...
    void sendNewUserJoinMessage_ProvideRightParam_Works() {
        ...
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile10", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
    ...
    void sendNewUserJoinMessage_ProvideWrongParam_Exception() {
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile11", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
        assertThrows(Exception.class, () -> {
    ...
    void removeUserFromViewingUsers_ProvideRightParam_Works() {
        ...
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile12", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
        List<String> currentViewingUsers = new ArrayList<>();
        currentViewingUsers.add("khader");
        docController.sendNewUserJoinMessage(doc.getId(), new CurrentViewingUserDTO("khader"));
    ...
    void removeUserFromViewingUsers_ProvideWrongParam_Exception() {
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile13", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
        assertThrows(Exception.class, () -> {
    ...
    void getPermission_ProvideRightParam_Works() {
        ...
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile16", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
    ...
    void getPermission_ProvideWrongParam_Exception() {
        ...
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile17", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
        assertThrows(Exception.class, () -> {
    ...
    void changeUserRollInDoc_ProvideRightParam_Works() {
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile19", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
    ...
    void changeUserRollInDoc_ProvideWrongParam_Exception() {
        ResponseEntity<Response<INode>> newFile = fileSystemController.addInode(new AddINodeDTO(userId, rootId, "khaderFile5", INodeType.FILE), token, userId);
        Document doc = (Document) newFile.getBody().getData();
        assertThrows(Exception.class, () -> {
